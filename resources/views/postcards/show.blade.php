            ...
            <aside class="item-sticky sticky top-8 space-y-10">
                <section class="rounded-2xl shadow-lg p-7 space-y-6 bg-[#697367] border border-black/20 p-4 mb-4">
                    <header class="space-y-3 mb-4 text-center">
                        <h1 class="text-2xl sm:text-3xl font-bold text-white leading-tight">{{ $postcard->card_title }}</h1>
                    </header>
                ...
                <section class="bg-[#697367] text-white rounded-2xl shadow-lg border border-black/20 p-8 lg:p-10 mb-4">
                    <dl class="space-y-6">
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            ...
                            <dd class="text-white">{{ $postcard->country?->name ?? '—' }}</dd>
                        </div>
                        @if($postcard->year)
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Year</dt>
                            <dd class="text-white">{{ $postcard->year }}</dd>
                        </div>
                        @endif
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Type</dt>
                            <dd class="text-white">{{ $postcard->postcardType?->name ?? '—' }}</dd>
                        </div>
                        @if($postcard->occasion)
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Occasion</dt>
                            <dd class="text-white">{{ $postcard->occasion }}</dd>
                        </div>
                        @endif
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Stamp status</dt>
                            <dd class="text-white">
                        ...
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            ...
                            <dd class="text-white">{{ $postcard->for_sale ? 'Yes' : 'No' }}</dd>
                        </div>
                        @if($postcard->selling_price !== null)
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Selling price</dt>
                            <dd class="text-white">€ {{ number_format($postcard->selling_price, 2, ',', '.') }}</dd>
                        </div>
                        @endif
                    </dl>
                </section>
                ...
                <section class="bg-[#697367] text-white rounded-2xl shadow-lg border border-black/20 p-8 lg:p-10">
                    ...
                    <dl class="space-y-6">
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            ...
                            <dd class="text-white">{{ $postcard->purchase_date ? $postcard->purchase_date->format('d/m/Y') : '—' }}</dd>
                        </div>
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Purchasing price</dt>
                            <dd class="text-white">{{ $postcard->purchasing_price !== null ? '€ ' . number_format($postcard->purchasing_price, 2, ',', '.') : '—' }}</dd>
                        </div>
                        @if($postcard->current_value !== null)
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Current value</dt>
                            <dd class="text-white">€ {{ number_format($postcard->current_value, 2, ',', '.') }}</dd>
                        </div>
                        @endif
                        @if($postcard->location)
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Location</dt>
                            <dd class="text-white">{{ $postcard->location->name }}</dd>
                        </div>
                        @endif
                        @if(!empty($postcard->personal_remarks))
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Personal remarks</dt>
                            <dd class="text-white/90 whitespace-pre-line">{{ $postcard->personal_remarks }}</dd>
                        </div>
                        @endif
