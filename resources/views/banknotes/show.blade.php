            ...
            <aside class="item-sticky sticky top-8 space-y-10">
                <section class="rounded-2xl shadow-lg p-7 space-y-6 bg-[#697367] border border-black/20 p-4 mb-4">
                    <header class="space-y-3 mb-4 text-center">
                        <h1 class="text-2xl sm:text-3xl font-bold text-white leading-tight">{{ $banknote->card_title }}</h1>
                    </header>
                ...
                <section class="bg-[#697367] text-white rounded-2xl shadow-lg border border-black/20 p-8 lg:p-10 mb-4">
                    <dl class="space-y-6">
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            ...
                            <dd class="text-white">{{ $banknote->country?->name ?? '—' }}</dd>
                        </div>
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Currency</dt>
                            <dd class="text-white">{{ $banknote->currency?->name ?? '—' }}</dd>
                        </div>
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Nominal value</dt>
                            <dd class="text-white">{{ $banknote->nominalValue?->name ?? '—' }}</dd>
                        </div>
                        @if($banknote->series)
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Series</dt>
                            <dd class="text-white">{{ $banknote->series->name }}</dd>
                        </div>
                        @endif
                        @if($banknote->timePeriod)
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Time period</dt>
                            <dd class="text-white">{{ $banknote->timePeriod->name }}</dd>
                        </div>
                        @endif
                        @if($banknote->year)
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Year</dt>
                            <dd class="text-white">{{ $banknote->year }}</dd>
                        </div>
                        @endif
                        @if($banknote->variation)
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Variation</dt>
                            <dd class="text-white">{{ $banknote->variation }}</dd>
                        </div>
                        @endif
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">For sale</dt>
                            <dd class="text-white">{{ $banknote->for_sale ? 'Yes' : 'No' }}</dd>
                        </div>
                        @if($banknote->selling_price !== null)
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Selling price</dt>
                            <dd class="text-white">€ {{ number_format($banknote->selling_price, 2, ',', '.') }}</dd>
                        </div>
                        @endif
                    </dl>
                </section>
                ...
                <section class="bg-[#697367] text-white rounded-2xl shadow-lg border border-black/20 p-8 lg:p-10">
                    ...
                    <dl class="space-y-6">
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            ...
                            <dd class="text-white">{{ $banknote->purchase_date ? $banknote->purchase_date->format('d/m/Y') : '—' }}</dd>
                        </div>
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Purchasing price</dt>
                            <dd class="text-white">{{ $banknote->purchasing_price !== null ? '€ ' . number_format($banknote->purchasing_price, 2, ',', '.') : '—' }}</dd>
                        </div>
                        @if($banknote->location)
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Location</dt>
                            <dd class="text-white">{{ $banknote->location->name }}</dd>
                        </div>
                        @endif
                        @if(!empty($banknote->personal_remarks))
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Personal remarks</dt>
                            <dd class="text-white/90 whitespace-pre-line">{{ $banknote->personal_remarks }}</dd>
                        </div>
                        @endif
