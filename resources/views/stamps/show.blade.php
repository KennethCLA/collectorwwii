            ...
            <aside class="item-sticky sticky top-8 space-y-10">
                <section class="rounded-2xl shadow-lg p-7 space-y-6 bg-[#697367] border border-black/20 p-4 mb-4">
                    <header class="space-y-3 mb-4 text-center">
                        <h1 class="text-2xl sm:text-3xl font-bold text-white leading-tight">{{ $stamp->card_title }}</h1>
                    </header>
                ...
                <section class="bg-[#697367] text-white rounded-2xl shadow-lg border border-black/20 p-8 lg:p-10 mb-4">
                    <dl class="space-y-6">
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            ...
                            <dd class="text-white">{{ $stamp->country?->name ?? '—' }}</dd>
                        </div>
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Currency</dt>
                            <dd class="text-white">{{ $stamp->currency?->name ?? '—' }}</dd>
                        </div>
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Nominal value</dt>
                            <dd class="text-white">{{ $stamp->nominalValue?->name ?? '—' }}</dd>
                        </div>
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Type</dt>
                            <dd class="text-white">{{ $stamp->stampType?->name ?? '—' }}</dd>
                        </div>
                        @if($stamp->year)
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Year</dt>
                            <dd class="text-white">{{ $stamp->year }}</dd>
                        </div>
                        @endif
                        @if($stamp->michel_number)
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Michel number</dt>
                            <dd class="text-white">{{ $stamp->michel_number }}</dd>
                        </div>
                        @endif
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">For sale</dt>
                            <dd class="text-white">{{ $stamp->for_sale ? 'Yes' : 'No' }}</dd>
                        </div>
                        @if($stamp->selling_price !== null)
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Selling price</dt>
                            <dd class="text-white">€ {{ number_format($stamp->selling_price, 2, ',', '.') }}</dd>
                        </div>
                        @endif
                    </dl>
                </section>
                ...
                <section class="bg-[#697367] text-white rounded-2xl shadow-lg border border-black/20 p-8 lg:p-10">
                    ...
                    <dl class="space-y-6">
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            ...
                            <dd class="text-white">{{ $stamp->purchase_date ? $stamp->purchase_date->format('d/m/Y') : '—' }}</dd>
                        </div>
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Purchasing price</dt>
                            <dd class="text-white">{{ $stamp->purchasing_price !== null ? '€ ' . number_format($stamp->purchasing_price, 2, ',', '.') : '—' }}</dd>
                        </div>
                        @if($stamp->current_value !== null)
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Current value</dt>
                            <dd class="text-white">€ {{ number_format($stamp->current_value, 2, ',', '.') }}</dd>
                        </div>
                        @endif
                        @if($stamp->location)
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Location</dt>
                            <dd class="text-white">{{ $stamp->location->name }}</dd>
                        </div>
                        @endif
                        @if(!empty($stamp->personal_remarks))
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Personal remarks</dt>
                            <dd class="text-white/90 whitespace-pre-line">{{ $stamp->personal_remarks }}</dd>
                        </div>
                        @endif
