        ...
        <div x-data="{ filtersOpen: false }" class="grid grid-cols-1 lg:grid-cols-[260px_minmax(0,1fr)] lg:gap-4 items-start">
            ...
            <div class="min-w-0 pr-4 pl-0">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-[240px_1fr] gap-4 items-center">
                        <div class="flex items-center justify-between">
                            <nav class="flex items-center pl-1 space-x-2 font-mono text-[11px] tracking-[0.15em] text-white/60 uppercase">
                                ...
                                <span class="text-khaki/70 font-mono text-[11px] tracking-[0.1em] uppercase">Stamps</span>
                            </nav>
                ...
                <div class="mb-4 flex flex-wrap items-center gap-2 bg-black/20 ring-1 ring-black/30 text-white rounded-xl px-3 py-2">
                    ...
                    <a href="{{ $remove('type_id') }}" class="inline-flex items-center gap-2 text-sm bg-sage hover:bg-[#5a6452] rounded-full px-3 py-1">
                ...
                <div class="bg-sage text-white rounded-md p-6">
                    ...
                    <div class="mt-4"><a href="{{ route('stamps.index') }}" class="inline-block rounded-xl bg-black/30 hover:bg-black/40 ring-1 ring-khaki/30 px-4 py-2 font-stencil tracking-[0.15em] text-sm text-white uppercase transition">Clear filters</a></div>
                </div>
                @else
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 h-full">
                    ...
                    <a href="{{ route('stamps.show', $stamp) }}" target="_blank"
                        ...
                        <div class="mb-1 flex-grow h-auto">
                            <p class="font-mono text-[9px] tracking-widest text-white/30 text-right mb-1">#{{ str_pad($stamp->id, 4, '0', STR_PAD_LEFT) }}</p>
                            <h3 class="text-lg font-bold text-center">{{ $stamp->card_title }}</h3>
                            @if($stamp->condition)
                            <p class="font-mono text-[9px] text-khaki/60 text-center mt-0.5 tracking-wider">{{ $stamp->condition }}</p>
                            @endif
                            @if($stamp->for_sale)<div class="flex justify-center mt-2"><span class="font-stencil text-[9px] tracking-[0.15em] text-khaki/65 border border-khaki/35 px-2 py-0.5 rotate-[-6deg] inline-block">ZU VERKAUFEN</span></div>@endif
                        </div>
