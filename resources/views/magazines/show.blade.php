...
<x-layout :mainClass="'mx-auto w-full max-w-none px-0 py-8'">
    ...
    <style>
        ...
        @media (max-width: 1100px) {
            ...
            .item-sticky { position: static !important; }
        }
    </style>
            ...
            <aside class="item-sticky sticky top-8 space-y-10">
                <section class="rounded-2xl shadow-lg p-7 space-y-6 bg-[#697367] border border-black/20 p-4 mb-4">
                    <header class="space-y-3 mb-4 text-center">
                        <h1 class="text-2xl sm:text-3xl font-bold text-white leading-tight">{{ $magazine->title }}</h1>
                        @if(!empty($magazine->subtitle))
                        <div class="h-px bg-white/20"></div>
                        <p class="text-white/80 italic">{{ $magazine->subtitle }}</p>
                        @endif
                    </header>
                ...
                @php
                $pdfUrl = $pdf->url();
                $pdfTitle = $pdf->original_name ? pathinfo($pdf->original_name, PATHINFO_FILENAME) : ('PDF ' . ($i + 1));
                @endphp
                @if ($pdfUrl)
                <section class="rounded-2xl shadow-lg p-7 space-y-5 bg-[#697367] border border-black/20 p-4">
                    <div class="flex items-start justify-between gap-4">
                        <div class="min-w-0">
                            <h2 class="text-lg sm:text-xl font-bold text-white truncate">{{ $pdfTitle }}</h2>
                        </div>
                        <a href="{{ $pdfUrl }}" target="_blank" rel="noopener"
                            class="shrink-0 text-sm px-3 py-2 rounded-md bg-white/10 text-white hover:bg-white/15 transition">Open</a>
                    </div>
                ...
                <section class="bg-[#697367] text-white rounded-2xl shadow-lg border border-black/20 p-8 lg:p-10 mb-4">
                    <dl class="space-y-6">
                        ...
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            ...
                            <dd class="text-white">{{ $magazine->publisher }}</dd>
                        </div>
                        @endif
                        ...
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            ...
                            <dd class="text-white">{{ $magazine->issue_number }}</dd>
                        </div>
                        @endif
                        ...
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            ...
                            <dd class="text-white">{{ $magazine->issue_year }}</dd>
                        </div>
                        @endif
                        ...
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            ...
                            <dd class="text-white whitespace-pre-line">{{ $magazine->description }}</dd>
                        </div>
                        @endif

                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">For sale</dt>
                            <dd class="text-white">{{ $magazine->for_sale ? 'Yes' : 'No' }}</dd>
                        </div>

                        @if($magazine->selling_price !== null)
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Selling price</dt>
                            <dd class="text-white">€ {{ number_format($magazine->selling_price, 2, ',', '.') }}</dd>
                        </div>
                        @endif
                    </dl>
                </section>
                ...
                <section class="bg-[#697367] text-white rounded-2xl shadow-lg border border-black/20 p-8 lg:p-10">
                    ...
                    <dl class="space-y-6">
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            ...
                            <dd class="text-white">{{ $magazine->purchase_date ? $magazine->purchase_date->format('d/m/Y') : '—' }}</dd>
                        </div>
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Purchase price</dt>
                            <dd class="text-white">{{ $magazine->purchase_price !== null ? '€ ' . number_format($magazine->purchase_price, 2, ',', '.') : '—' }}</dd>
                        </div>
                        @if(!empty($magazine->notes))
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Notes</dt>
                            <dd class="text-white/90 whitespace-pre-line">{{ $magazine->notes }}</dd>
                        </div>
                        @endif
