            ...
            <aside class="item-sticky sticky top-8 space-y-10">
                <section class="rounded-2xl shadow-lg p-7 space-y-6 bg-[#697367] border border-black/20 p-4 mb-4">
                    <header class="space-y-3 mb-4 text-center">
                        <h1 class="text-2xl sm:text-3xl font-bold text-white leading-tight">{{ $newspaper->title }}</h1>
                    </header>
                ...
                <section class="bg-[#697367] text-white rounded-2xl shadow-lg border border-black/20 p-8 lg:p-10 mb-4">
                    <dl class="space-y-6">
                        ...
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            ...
                            <dd class="text-white">{{ $newspaper->publisher }}</dd>
                        </div>
                        @endif
                        @if($newspaper->publication_date)
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Publication date</dt>
                            <dd class="text-white">{{ $newspaper->publication_date->format('d/m/Y') }}</dd>
                        </div>
                        @endif
                        @if(!empty($newspaper->description))
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Description</dt>
                            <dd class="text-white whitespace-pre-line">{{ $newspaper->description }}</dd>
                        </div>
                        @endif
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">For sale</dt>
                            <dd class="text-white">{{ $newspaper->for_sale ? 'Yes' : 'No' }}</dd>
                        </div>
                        @if($newspaper->selling_price !== null)
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Selling price</dt>
                            <dd class="text-white">€ {{ number_format($newspaper->selling_price, 2, ',', '.') }}</dd>
                        </div>
                        @endif
                    </dl>
                </section>
                ...
                <section class="bg-[#697367] text-white rounded-2xl shadow-lg border border-black/20 p-8 lg:p-10">
                    ...
                    <dl class="space-y-6">
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            ...
                            <dd class="text-white">{{ $newspaper->purchase_date ? $newspaper->purchase_date->format('d/m/Y') : '—' }}</dd>
                        </div>
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Purchase price</dt>
                            <dd class="text-white">{{ $newspaper->purchase_price !== null ? '€ ' . number_format($newspaper->purchase_price, 2, ',', '.') : '—' }}</dd>
                        </div>
                        @if(!empty($newspaper->notes))
                        <div class="rounded-xl border border-white/10 bg-black/20 px-6 py-2 mb-4">
                            <dt class="text-sm text-white/70 font-semibold">Notes</dt>
                            <dd class="text-white/90 whitespace-pre-line">{{ $newspaper->notes }}</dd>
                        </div>
                        @endif
