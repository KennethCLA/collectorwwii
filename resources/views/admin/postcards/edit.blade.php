...
@section('admin-content')
        ...
        <form id="postcard-form" action="{{ route('admin.postcards.update', $postcard) }}" method="POST" class="w-full mx-auto max-w-7xl">
            @csrf
            @method('PUT')
            ...
            <div class="rounded-xl border border-black/20 bg-black/10 p-6 space-y-8">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="condition" class="w-full rounded-md border border-black/30 bg-white/10 px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-white/20">
                                ...
                                <option value="{{ $grade }}" @selected(old('condition', $postcard->condition ?? '') === $grade)>{{ $grade }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <section class="rounded-xl border border-white/10 bg-black/20 p-6">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <input type="date" name="purchase_date" value="{{ old('purchase_date', $postcard->purchase_date?->format('Y-m-d') ?? '') }}"
                        ...
                        {{-- Sold --}}
                        <div x-data="{ sold: {{ old('sold_at', $postcard->sold_at ?? null) ? 'true' : 'false' }} }" class="space-y-2">
                            ...
                            <div x-show="sold" x-cloak class="grid grid-cols-2 gap-3 pt-2">
                                <div>
                                    ...
                                    <input type="date" name="sold_at"
                                        value="{{ old('sold_at', $postcard->sold_at?->format('Y-m-d') ?? '') }}"
                                ...
                                <div>
                                    ...
                                    <input type="number" step="0.01" name="sold_price"
                                        value="{{ old('sold_price', $postcard->sold_price ?? '') }}"
                    ...
                    <div class="mt-6 space-y-2">
                        ...
                        <textarea name="personal_remarks" rows="4"
                            class="w-full rounded-md border border-black/30 bg-white/10 px-3 py-2 text-white placeholder:text-white/40 focus:outline-none focus:ring-2 focus:ring-white/20">{{ old('personal_remarks', $postcard->personal_remarks) }}</textarea>
                    </div>
                </section>
                ...
                <section class="rounded-xl border border-black/20 bg-black/10 p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="rounded-md bg-sage-900 p-4 border border-white/10">
                            ...
                            <form action="{{ route('admin.media.store', ['type' => 'postcards', 'id' => $postcard->id]) }}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-3">
                                @csrf
                                <input type="hidden" name="collection" value="images">
                                <input type="file" name="files[]" multiple accept="image/*" class="w-full rounded-md border border-black/30 bg-white/10 px-3 py-2 text-white file:mr-3 file:rounded-md file:border-0 file:bg-white/10 file:px-3 file:py-2 file:text-white hover:file:bg-white/15">
                        ...
                        <div class="rounded-md bg-sage-900 p-4 border border-white/10">
                            ...
                            <form action="{{ route('admin.media.store', ['type' => 'postcards', 'id' => $postcard->id]) }}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-3">
                                ...
                                <input type="file" name="files[]" multiple accept="application/pdf" class="w-full rounded-md border border-black/30 bg-white/10 px-3 py-2 text-white file:mr-3 file:rounded-md file:border-0 file:bg-white/10 file:px-3 file:py-2 file:text-white hover:file:bg-white/15">
                    ...
                    @php
                    $mediaImages = $postcard->images;
                    $mediaPdfs = $postcard->files->filter(fn($f) => $f->isPdf())->values();
                    @endphp
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <div class="bg-sage rounded-md p-4 border border-black/20">
                            ...
                            <div class="flex flex-wrap gap-2 items-start"
                                ...
                                data-reorder-url="{{ route('admin.media.reorder', ['type' => 'postcards', 'id' => $postcard->id]) }}">
                ...
                <div class="flex items-center justify-end gap-3 pt-2">
                    ...
                    <button type="submit" form="postcard-form" class="rounded-md bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-500">Save changes</button>
                </div>
            </div>
